...
const createNewOrder = async (req, res) => {
    ...
    try {
        ...
        for (let i = 0 ; i < listProduct.length; i++) {
            let message2 = await dathangService.CreateNewOrderProduct([listProduct[i][0], idcode, listProduct[i][1]]); 
...
const productIntoCategory = async (req, res) => {
    try {
        ...
        for (let i = 0 ; i < listCategory.length; i++) {
            ...
            propertyValue = await feature.ProductIntoCategory([listCategory[i].maLoai])
